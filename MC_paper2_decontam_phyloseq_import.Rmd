...
RECps <- dget("function/FUN_RECps.R") # Read in RECps function
...
samdf1 <- read.csv(file.path("data/MC_paper2_samdf_BCMC1_total_lyse.csv"))
row.names(samdf1) <- samdf1$SampleID

# 2. DNA extraction kit with MoBio
samdf2 <- read.csv(file.path("data/MC_paper2_samdf_BCMC1_MoBio.csv"))
row.names(samdf2) <- samdf2$SampleID

# 3. DNA extraction kit choice on 10 ml UHT milk; BCMC2
samdf3 <- read.csv(file.path("data/MC_paper2_samdf_BCMC2_kit_10ml.csv"))
row.names(samdf3) <- samdf3$SampleID

# 4. PMA treatment
samdf4 <- read.csv(file.path("data/MC_paper2_samdf_PMA.csv")) 
row.names(samdf4) <- samdf4$SampleID

# 5. Sample storage
samdf5 <- read.csv(file.path("data/MC_paper2_samdf_sample_storage.csv"))
row.names(samdf5) <- samdf5$SampleID

# 6. Sample Volume
samdf6 <- read.csv(file.path("data/MC_paper2_samdf_total_vol.csv"))
row.names(samdf6) <- samdf6$SampleID

# 7. phase 3 kit, cleanup and lysing method 
samdf7 <- read.csv(file.path("data/phase3_samdf.csv"))
row.names(samdf7) <- samdf7$SampleID

# 8. UHT milk negative control
samdf8 <- read.csv(file.path("data/MC_paper2_mapping_UHT.csv"))
